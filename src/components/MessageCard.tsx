...
export default function MessageCard({ title, name, images, preview }: MessageCardProps) {
  ...
  return (
    <article className="group relative overflow-hidden rounded-3xl bg-white shadow-xl transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 border border-gray-100">
      ...
      <h3 className="relative z-10 px-8 pt-8 text-sm font-bold uppercase tracking-wider text-aps-navy">
        {title}
      </h3>
      <div className="mt-4 grid md:grid-cols-5">
        <div className="flex items-center justify-center bg-gradient-to-br from-aps-navy to-aps-magenta p-8 md:col-span-2">
          <span className="rounded-2xl border-2 border-white/30 bg-white/10 backdrop-blur-sm px-6 py-3 text-xs font-bold uppercase tracking-wider text-white transition-all duration-300 group-hover:scale-105">
            Read Message →
          </span>
        </div>
      ...
      <div className="border-t border-gray-100 px-8 py-6">
        <p className="text-lg font-semibold text-aps-navy">{name}</p>
        <p className="mt-3 text-gray-600 leading-relaxed line-clamp-3">{preview}</p>
      </div>
    </article>
  );
}
